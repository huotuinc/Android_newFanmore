...
public class SinaShareActivity extends BaseActivity implements OnkeyBackListener{
	...
	private final String APP_SECRET="0783d8dd1f0eb5a45687cde79aa10108";
   ...
   // private static final String WEIBO_DEMO_APP_SECRET = "4ecff164368e17c8a417ef49a6e755ad";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 获取 Token 成功
		String response = PreferenceHelper.readString(SinaShareActivity.this, Constant.SP_NAME_NORMAL, Constant.SP_NAME_SINA_TOKEN);
...
//			}
    ...
    class AuthListener implements WeiboAuthListener {
        ...
        public void onComplete(Bundle values) {
			...
			//fetchTokenAsync(code, Constant.SINA_KEY_SECRET);
			//fetchTokenAsync(code, "0783d8dd1f0eb5a45687cde79aa10108");
			fetchTokenAsync(code, APP_SECRET);						
			
        }

        @Override
        public void onCancel() {
        	ToastUtils.showLongToast(SinaShareActivity.this, "取消认证");
        	finish();
        }

        @Override
        public void onWeiboException(WeiboException e) {
        	ToastUtils.showLongToast(SinaShareActivity.this , "Auth exception : " + e.getMessage());
        	finish();
        }
    }
	...
	private void share(){
		...
		if (bundle == null)
			...
			LayoutInflater inflater= LayoutInflater.from(SinaShareActivity.this);		
			View layout = inflater.inflate(R.layout.custom_dialog_content_sina, null);
			...
			final Bitmap bitmap = LruImageCache.instance().getBitmap(path);

			txtDes.setText(des);
			img.setBackgroundDrawable(new BitmapDrawable(bitmap));

			CustomDialog.setOnKeyBackListener(SinaShareActivity.this);
			CustomDialog.showChooiceDialg( SinaShareActivity.this, "" , "" , "发送", "返回", layout,
					new DialogInterface.OnClickListener() {
						...
						@Override
						public void onClick(DialogInterface dialog, int which) {
							ToastUtils.showLongToast(SinaShareActivity.this,"取消发送");
							finish();
						}
					});
    ...
    private void sharenoui(){
        ...
        mStatusesAPI = new StatusesAPI(mAccessToken); //创建微博分享接口实例
        String content =String.format("【%s】%s", des,  url);
        //mStatusesAPI.upload(content, bitmap, null, null, mListener);
        //mStatusesAPI.update(content, null, null, mListener);
        if( bmp !=null ){
            mStatusesAPI.upload(content, bmp, null, null, mListener);
        }
        else 
        {
            mStatusesAPI.update(content, null, null, mListener);
            ...
            if (!TextUtils.isEmpty(response)) {
                if(response.startsWith("{\"access_token\"")){
                    ...
                    if (token != null && token.isSessionValid()) {
                        mAccessToken = token;
                        PreferenceHelper.writeString(SinaShareActivity.this, Constant.SP_NAME_NORMAL, Constant.SP_NAME_SINA_TOKEN, response);
                        //share();
                        
                        sharenoui();
                        
                    } else {
                       ToastUtils.showLongToast(SinaShareActivity.this , "Failed to receive access token");
                    }
                }else if (response.startsWith("{\"created_at\"")) {
                    // 调用 Status#parse 解析字符串成微博对象
                    //Status status = Status.parse(response);
                    //ToastUtils.showLongToast(SinaShareActivity.this , "分享成功");
...
//                  //send broadcast
	...
	public void onKeyBack() {
		//ToastUtils.showLongToast(this,"取消分享");
		finish();
